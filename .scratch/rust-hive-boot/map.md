...
- [POST a Load with Idempotency-Key](issues/08-create-load-post.md) — `POST /loads`; required `Idempotency-Key`; `loads::new` + `router`; replay on matching fingerprint; `23505` is `LOAD_CONFLICT`.
